...
function ask(prompt: string): Promise<string> {
  return new Promise((res) => rl.question(chalk.blue(`👤 ${prompt}\n> `), res));
}
...
function printBanner() {
  ...
  `));
  console.log(chalk.yellow("An intelligent research agent with ambiguity detection and clarification capabilities\n"));
}
...
async function main() {
  ...
  while (true) {
    ...
    try {
      ...
      if (!ambiguity || !ambiguity?.isAmbiguousQuery) {
        console.log(chalk.green("✅ Query is clear and unambiguous. Proceeding to research..."));
        break;
      }
      
      if (context.iterationCount >= MAX_CLARIFICATION_ITERATION) {
        console.log(chalk.yellow("⚠️  Maximum clarification iterations reached. Proceeding with current query..."));
        break;
      }
  ...
  try {
    ...
    planningSpinner.succeed(chalk.green(`Research plan created with ${context.searches.length} search strategies`));
  } catch (error) {
  ...
  const executionSpinner = ora({
    text: chalk.gray(`Executing ${context.searches.length} searches in parallel...`),
    spinner: "clock"
  }).start();
  ...
  try {
    ...
    executionSpinner.succeed(chalk.green(`Search execution completed. Retrieved ${context.searchResults.length} result sets`));
  } catch (error) {
  ...
  console.log(chalk.yellow("Next steps: Implement the report generation agent to synthesize these results."));
